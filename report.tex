...
%     - pred_examples.png   : paired success (top) vs failure (bottom)
%                             keyframes from the worst held-out signer with
%                             MediaPipe skeleton overlay; caption gives
%                             predicted vs true gloss.
% =====================================================================
...
\textbf{Open challenge from the survey.} The reference survey for Domain~C (Zhou \emph{et al.}, 2023~\cite{zhou2023}) lists \emph{cross-subject generalisation} as one of the unsolved problems in pose-based action recognition: models trained on one set of subjects tend to overfit to body proportions, signing style and camera placement, and drop on people they were not trained on. In the sign-language specific setting, the SLR survey of De Coster \emph{et al.}~\cite{decoster2023survey} makes the same point under the label \emph{cross-signer generalisation}. On WLASL~\cite{li2020}, which has few examples per gloss and a non-uniform distribution of signers per class, this is the dominant generalisation gap.
...
Pose-based methods take the opposite route. ST-GCN~\cite{yan2018} hard-codes a skeletal adjacency, awkward when fingers carry most of the discriminative signal. The Sign Pose-based Transformer of Bohacek and Hr\'uz~\cite{bohacek2022sptn} drops the graph and runs a per-frame Transformer over MediaPipe landmarks, reaching $\sim$63\% Top-1 on the standard (signer-overlapping) WLASL100 split; this is the closest published architecture to mine. OpenHands~\cite{selvaraj2022openhands} releases pose-based pretrained baselines and discusses generalisation across signers as a remaining issue, though it does not benchmark a strict signer-disjoint protocol. The gap that motivates this work: published pose-based numbers are almost always on signer-overlapping splits, so the headline accuracy hides how much is identity rather than sign. I close that loophole by re-splitting WLASL100 strictly by signer. Standard ingredients: Transformer encoder~\cite{vaswani2017attention}, AdamW~\cite{loshchilov2019adamw}, MediaPipe Holistic~\cite{lugaresi2019mediapipe}.
...
Per frame I take 75 MediaPipe Holistic joints (33 body + 21+21 hand) in 3D, apply RQE (centre on shoulder midpoint, divide by inter-shoulder distance, clip to $\pm 20$, fallback scale 1.0 if torso is degenerate), and flatten to a 225-d vector. Then linear projection to $d_\text{model}=128$, learned positional embedding (max length 64), 2-layer / 4-head pre-norm Transformer (FFN 256, dropout 0.2, GELU), masked temporal mean pool, linear head $\to$ 100 logits. About 430k parameters total --- deliberately small to match the few-shot training-set size.

\textbf{Why this should help cross-signer generalization.} RQE at the input kills the two cleanest identity signals (position, body size) before the network sees them --- a representational prior, not something the model has to learn from a small training set. Self-attention then weights hand-shape transitions \emph{by content}, which is more tolerant of speed variation than a fixed-window convolution would be; the learned \emph{absolute} positional embedding does still tie attention to absolute frame indices, which is the limitation that resurfaces in \S\ref{sec:fail}. No fixed skeletal graph (unlike ST-GCN) means no signer-specific corrections baked into GCN weights.

\textbf{Baseline (Pooled-MLP).} Same RQE input, masked temporal mean, then a 2-layer MLP. This is intentionally a weak baseline: it has no temporal modelling at all. The comparison therefore measures whether \emph{any} temporal model adds value on top of RQE, not whether attention specifically beats a different temporal backbone. A stronger baseline (e.g.\ temporal CNN or LSTM) would isolate attention more sharply but was out of scope for the matched-compute budget of this project.
...
\textbf{Dataset.} WLASL100 (top-100 glosses). The chinhde Kaggle landmarks dump turned out to be incompatible with the official WLASL metadata (no \texttt{video\_id}, mismatched per-class counts), so I extracted landmarks directly from the raw videos in \texttt{risangbaskoro/wlasl-processed} using MediaPipe Holistic~\cite{lugaresi2019mediapipe}. Of the 2{,}038 WLASL100 video IDs in \texttt{nslt\_100.json}, only \todo{N\_present}/2038 ($\approx$50\%) were actually reachable on disk --- the rest have dead source URLs, a known WLASL issue. \textbf{Effective WLASL100 in this project is therefore about half the published benchmark}, and absolute numbers are not comparable to public leaderboards (the internal comparison between rows of Table~\ref{tab:results} is still fair because both share the same input and recipe).

\textbf{Note on RQE and the \boldmath$z$-axis.} MediaPipe Holistic returns $z$-coordinates in landmark-group-specific reference frames (pose $z$ relative to the hip, hand $z$ relative to the wrist), so they are not directly comparable across joints. RQE's inter-shoulder scaling is a planar (mostly $x$) distance, so applying it to $z$ does not produce a clean invariance. We keep $z$ in the input and let the linear projection learn to downweight it; an alternative (dropping $z$ entirely) is left for the next iteration.
...
\textbf{Training configuration.} AdamW lr $3\!\times\!10^{-4}$, weight decay $1\!\times\!10^{-4}$, cosine schedule, batch size 32, max 64 frames per clip, 30 epochs, cross-entropy with label smoothing 0.1, grad-clip 1.0. Trained from scratch (no pretraining). No validation-based early stopping or best-checkpoint selection: we report the end-of-training model and rely on the cosine schedule for a smooth landing. Each model is trained with 3 random seeds (0, 1, 2); I report mean $\pm$ std on the held-out test set.

\textbf{Train-time augmentation.} Three light augmentations on the train split only (val/test see clean inputs): horizontal flip with probability 0.5 (mirrors $x$ after RQE, so the model sees both handednesses), additive Gaussian noise on landmarks with $\sigma{=}0.01$ torso units, and random temporal start when the clip is longer than 64 frames (otherwise centred crop). Picked after a no-augmentation run collapsed to near-chance on the held-out signers.
...
\begin{subfigure}{0.48\linewidth}
  ...
  \caption{Train loss / val Top-1 over 30 epochs (seed 0).}
  \label{fig:loss}
\end{subfigure}
\caption{RQE-Transformer on the signer-disjoint test set.}
\end{figure}

\textbf{Comparison with published numbers.} Pose-based methods with pretraining reach 60--80\% Top-1 on the \emph{standard} WLASL100 split~\cite{selvaraj2022openhands,bohacek2022sptn}. My setup is intentionally weaker (no pretraining, only light landmark-level augmentation, no hand-crops, half the videos missing, signer-disjoint split is harder), so I expect to be well below that. The point is the internal comparison between the two rows in Table~\ref{tab:results}, not the absolute number.
...
The biggest off-diagonal cell of the confusion matrix (Figure~\ref{fig:cm}) is \todo{gloss A vs gloss B} (\todo{xx\%} of A misclassified as B). The two glosses share the same gross arm trajectory and differ in hand-shape. MediaPipe hand landmarks are in the input --- fingers contribute $\sim$$\tfrac{42 \times 3}{225}\!\approx\!56\%$ of the per-frame vector \emph{by count} --- but I \emph{hypothesise} they contribute much less in \emph{magnitude} after the linear projection, because most across-frame variance lives in the body channels and the projection is unconstrained. After the masked temporal mean pool, the already-weaker finger signal gets averaged across time. This is consistent with a representational limit of pose-only input at MediaPipe's resolution rather than a learning failure --- ST-GCN would have the same input-bandwidth problem. A direct test (plotting per-joint $\|\mathbf{W}_\text{proj}\|$ contributions) is left to the next iteration.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{pred_examples.png}
\caption{Paired success (top) vs failure (bottom) clips from the same worst held-out signer, 4 keyframes each, MediaPipe skeleton overlaid. Top: correctly classified \todo{gloss}. Bottom: predicted \todo{X}, true \todo{Y}. The contrast localises the failure mode --- the model is not broken on this signer overall; specific signs survive while others don't.}
\label{fig:fail}
\end{figure}
...
\textbf{Feasibility.} All ingredients are preprocessing-only or additive on top of the current code: the hand landmarks are already in the cached \texttt{wlasl100\_landmarks.npz}; dominant-hand detection is a single pass over hand-motion energy; the extra Transformer adds $<$200k parameters relative to the current $\sim$430k, still well within the same T4 training budget; evaluation is on the same signer-disjoint split, so the comparison is direct and the effect on Case 2 (and Case 1, via the canonicalisation) is directly measurable.
